# app.py — single, fast HA/HB/HC app with a visible sidebar chatbot (Azure ENV-first)
import os
import re
import pandas as pd
...
APP_VERSION = "v-weekpicker-generic-alarm-2025-09-22-OPT2-chat-azureenv"
...
# ---------- config helpers (Azure ENV-first) ----------
def _cfg(key: str, default: str = "") -> str:
    ...
    if val is None:
        try:
            val = st.secrets.get(key)  # available on Streamlit Cloud/local only
        except Exception:
            val = None
    return (val if val is not None else default).strip()

def smart_read_csv(path_or_url: str, **kwargs) -> pd.DataFrame:
    """Read CSV from local path or HTTPS/Azure Blob SAS URL."""
...
def initialize_llm():
    ...
    # If any are missing, return None so the app runs without AI
...
def main():
    ...
    with st.sidebar:
        ...
        # Data sources via ENV (Azure) or local fallback
        tele_file = _cfg("TELEMATICS_URL", _cfg("TELEMATICS_PATH", "telematics_new_data_2207.csv"))
        excl_file = _cfg("EXCLUSIONS_URL", _cfg("EXCLUSIONS_PATH", "vehicle_exclusions.csv"))
        head_file = _cfg("HEADCOUNTS_URL", _cfg("HEADCOUNTS_PATH", "depot_headcounts.csv"))
    ...
    w2_row = weekly_sum[(weekly_sum["alarm_year"]=_]()_]()
