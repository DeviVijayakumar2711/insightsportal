# app.py — single, fast HA/HB/HC app with a visible sidebar chatbot (cloud-first, all alarms)
import os
import re
...
APP_VERSION = "v-weekpicker-alarm-unified-chat-2025-10-03"
...
def calc_metrics(ev_df: pd.DataFrame, trips_df: pd.DataFrame, category: str) -> pd.DataFrame:
    ...
    if not trips_df.empty:
        ...
        dur = td.groupby(category, sort=False)["trip_duration_hr"].sum().rename("Total Duration (hr)")
    else:
...
def generate_ai_deep_dive(_llm, alarm_code, w1_metric, delta, driver_perf, bus_perf, svc_perf, fleet_avg_per_trip, _v=APP_VERSION):
    ...
    def slim(df, keep_cols, n=5):
        ...
        d = df[keep_cols].copy()
        for c in d.columns:
            if pd.api.types.is_numeric_dtype(d[c]): d[c] = d[c].round(3)
    ...
    try:
        return _llm.predict(
            f"""You are an expert Bus Operations Analyst. Alarm: {alarm_code}.
Use ONLY this JSON summary to produce insights.
...
# ---------- Chatbot helpers (multi-alarm aware) ----------
def _parse_query(q: str) -> Tuple[str, Optional[str], int, Optional[int], Optional[int]]:
    ...
    """
    if not q: return "generic", None, 12, None, None
    ql = q.strip().lower()
    ...
    if "all drivers" in ql or re.search(r"\bdrivers\b", ql):  return "drivers", None, n_weeks, wk, yr
    if "all buses" in ql or re.search(r"\bbuses\b", ql):      return "buses", None, n_weeks, wk, yr
    if "all services" in ql or re.search(r"\bservices\b", ql):return "services", None, n_weeks, wk, yr
    ...
    for pat, etype in [
        ...
        m = re.search(pat, ql)
        if m: return etype, m.group(1).upper(), n_weeks, wk, yr
...
def answer_entity_question(q: str, alarm_choice: str, df_alarm: pd.DataFrame, llm=None):
    """
    Uses the CURRENT alarm_choice (HA/HB/HC) and df_alarm already filtered to that alarm,
    so this works identically on HB & HC as on HA.
    """
    ...
    if etype in {"drivers","buses","services"}:
        ...
        return f"Top {etype} in the last **{n_weeks}** weeks (ranked by *Alarms per Trip*) — Alarm: **{ALARM_MAP[alarm_choice]['short']}**", scope_top
    ...
    if etype == "generic":
        ...
        ans = (
            f"**Fleet summary** for **{ALARM_MAP[alarm_choice]['short']}** (last **{n_weeks}** weeks):\n"
        ...
        if llm is not None:
            try:
                stats_json = {
                    "alarm": ALARM_MAP[alarm_choice]["short"],
                    "n_weeks": n_weeks,
...
def main():
    ...
    with chat_box:
        ...
        st.caption("Examples: `driver 30450`, `bus SMB1464Z`, `svc 973`, `all drivers last 8 weeks`, `all buses`")
        ...
        if clear_clicked: st.session_state.qa = []
        st.markdown("---")
    ...
    with st.sidebar:
        ...
        if not tele_file or not excl_file or not head_file:
            st.error("Data file paths are not configured. Set TELEMATICS_URL/PATH, EXCLUSIONS_URL/PATH, HEADCOUNTS_URL/PATH.")
            return
    ...
    st.title(f"Weekly {ALARM_MAP[alarm_choice]['long']} Review")
    ...
    # Process chat AFTER slice so it respects alarm_choice
    with chat_box:
        if pending_q:
    ...
    st.markdown(f"## 📈 Performance Snapshot: Week {w1_week}, {w1_year}")
    c1, c2 = st.columns(2)
    c1.metric(f"Avg {ALARM_MAP[alarm_choice]['short']} per BC (W{w1_week})", f"{w1_metric:.2f}", delta=delta_text, delta_color="inverse")
    c2.metric(f"Total {ALARM_MAP[alarm_choice]['short']} Incidents", f"{w1_events_count:,.0f}",
    ...
    st.plotly_chart(
        trend_chart(chart_data, "per_bc", f"Avg {ALARM_MAP[alarm_choice]['short']} per Bus Captain", alarm_choice),
    ...
    if initialize_llm() is None:
        st.info("AI model is not configured.")
    elif w1_events.empty:
        st.info(f"No {ALARM_MAP[alarm_choice]['short']} rows for the selected week.")
    else:
        with st.expander("Context for AI (counts)", expanded=False):
            st.write(f"Rows (events): **{len(w1_events):,}** | Unique trips: **{trips_unique['trip_id_norm'].nunique():,}**")
        if st.button("Generate AI Analysis for Selected Week", type="primary"):
            with st.spinner("Generating insights..."):
                ...
                ai_md = generate_ai_deep_dive(initialize_llm(), alarm_choice, w1_metric, delta, driver_perf, bus_perf, svc_perf, fleet_avg)
                st.markdown(ai_md or "")

    if w1_events.empty:
        st.caption("Note: No matching event rows found. Tables will be empty.")

    # Manual tables
    st.markdown("---")
    st.subheader(f"🔬 Manual Analysis Tables: Week {w1_week}")
    ...
    with tab1:
        ...
        st.dataframe(df_dr.sort_values(["Alarms per Trip","Alarm Count"], ascending=[False,False])[cols_show].round(2)) if not df_dr.empty else st.info("No driver events this week.")
    with tab2:
        ...
        st.dataframe(df_bs.sort_values(["Alarms per Trip","Alarm Count"], ascending=[False,False])[cols_show].round(2)) if not df_bs.empty else st.info("No bus events this week.")
    with tab3:
        ...
        st.dataframe(df_sv.sort_values(["Alarms per Trip","Alarm Count"], ascending=[False,False])[cols_show].round(2)) if not df_sv.empty else st.info("No service events this week.")
