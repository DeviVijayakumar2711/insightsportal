...
def load_and_process_data(telematics_file, exclusions_file, headcounts_file, file_mtime, _v=APP_VERSION):
    ...
    df_raw["depot_id_norm"] = df_raw["depot_id"].astype(str).str.strip().astype("category")  # <-- fixed
...
def calc_metrics(ev_df: pd.DataFrame, trips_df: pd.DataFrame, category: str) -> pd.DataFrame:
    ...
    trips_unique = (
        ev_df[["trip_id_norm", category]].drop_duplicates()
    ...
    if not trips_df.empty:
        ...
        dur = (
            td.groupby(category, sort=False)["trip_duration_hr"].sum()
    ...
    with tab1:
        ...
        if df_dr.empty:
            ...
            for c in ["Alarm Count","Alarm Trips","Alarms per Trip","Alarms per Hour","Total Duration (hr)"]:
                if c in view.columns: view[c] = pd.to_numeric(view[c], errors="coerce")
            view = view.round(2)
